...
const AddTransaction = ({ coins }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [search, setSearch] = useState('');
    const [selectedCoin, setCoin] = useState('');
    const [selectedButton, setSelectedButton] = useState('buy');
    const [inputValue, setInputValue] = useState(''); // початкове значення може бути встановлене змінною selectedCoin.current_price



    useEffect(() => {
        if (selectedCoin) {
            setInputValue(selectedCoin.current_price || '');
        }
    }, [selectedCoin]);
